...
namespace StarMon.Hardware.Bios
{
    ...
    public abstract class BiosData
    {
        ...
        {
            Default = 0,  // 0x00 = 0b00000000
            Performance = 1,  // 0x01 = 0b00000001
            Cool = 2,  // 0x02 = 0b00000010
            Quiet = 3,  // 0x03 = 0b00000011
            Extreme = 4,  // 0x04 = 0b00000100
        ...
        public struct FanTable
        {
            ...
            public FanTable(byte[] data)
            {
                ...
                // As for the color table, the level count is firmware-supplied
                // and is clamped to what the response can actually contain.
                int fits = data == null ? 0 : (data.Length - 2) / 3;
                if (fits < 0)
                    fits = 0;
