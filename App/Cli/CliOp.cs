...
#region BIOS Information Retrieval
                                // One refused operation must not end the report.
                                //
                                // `-Bios` with no arguments runs twenty-one
                                // read-only operations in turn, and the firmware
                                // refuses whichever ones the model does not
                                // implement — GetGpuPower on a machine with no
                                // configurable graphics power, for one. The
                                // exception used to propagate out of here and out
                                // of Main, so the run stopped dead at the first
                                // refusal and every operation after it went
                                // unreported. Which calls a machine refuses is
                                // precisely what this report is for, so a refusal
                                // is a result to print, not a reason to stop.
                                try {
                                ...
                                switch(args[i].ToLowerInvariant()) {
                                    ...
                                    // Named, so the report says which call this
                                    // machine would not answer rather than
                                    // simply stopping partway through
                                    Cli.PrintError(args[i] + ": " + opError.Message);
