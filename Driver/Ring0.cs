...
#region Driver Setup Methods
        ...
        private static bool Extract(string filePath) {
            ...
            try {
                using Stream stream = assembly.GetManifestResourceStream("StarMon.Driver.sys.gz");
                if(stream != null) {
                    using FileStream target = new(filePath, FileMode.Create);
                    // The embedded resource carries one leading guard byte
                    // (0xFF) ahead of the gzip stream, so the file does not
                    // start with the 1F 8B magic number that would make an
                    // extracted copy identifiable as a compressed driver.
                    // Verified against the resource: byte 0 is 0xFF and the
                    // gzip header begins at offset 1. Removing this line
                    // corrupts the header and disables the driver silently,
                    // because the failure is swallowed by the catch below.
                    stream.Position = 1;
